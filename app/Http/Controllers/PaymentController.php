...
public function pay(){
   ...
   protected function studentPay(Request $request){
    ...
    $studentApplied = StudentAppliedHistory::firstOrNew([
                                            'college_id'=>$college->reg_id,
                                            'student_id'=>$this->student->student_id,
                                            'course_dept_id'=>$course->id,]);
    if(is_null($studentApplied->id)){

            $studentApplied->college_id= $college->reg_id;
            $studentApplied->student_id=$this->student->student_id;
    ...
    //dd($studentApplied);
        if($studentApplied->save()){
            ...
            $data = [
                ...
                'firstname' => $this->student->first_name, # Payee Name.
                'firstname' => $this->student->last_name, # Payee Name.
                'email' => $this->student->email, # Payee Email Address.
                'phone' => $this->student->mobile, # Payee Phone Number.
            ];
            return Payment::make($data, function($then) {
             
                $then->redirectTo('studentpaystatus');
            });
        }
    }
    return redirect()->back()->withErrors(['status'=>'warning','message'=>'You have Already Applied to this Course']);
   ...
   protected function studentStatus(){
        ...
        $array = json_decode($payment, true);
         //dd($array);
        $reg_array = array('account_id'=>$this->student->student_id);
        if(PayuPayment::where('txnid',$array['txnid'])->count() == 0){
            PayuPayment::create($array+$reg_array);
            if($array['txnid']=='success')
            {
                Student::where('student_id',$this->student->student_id)->update(['compilation_status'=>'Done']);
            }
        }
        return view('student.pages.empty',['payment'=>$payment]);
    }
}
